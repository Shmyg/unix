...
d_start() {
  ...
  # Replaced with the old command by Shmyg 22.04.2021
  su -c "screen -ls | grep -sq "\.${srnname}[[:space:]]" " ${user} || su -c "screen -S ${srnname} -d -m rtorrent 2>&1 1>/dev/null" ${user} | tee -a "$logfile" >&2
...
getsession() { 
    ...
    # Modifified by Shmyg 22.04.2021, session directory hardcoded
    #echo $session
    echo '/home/shmyg/rtorrent/.session'
}
